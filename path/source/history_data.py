...
source = urllib.request.urlopen(
    f"https://www.formula1.com/en/results.html/1990/races/64/united-states/race-result.html").read()
soup = BeautifulSoup(source, 'html.parser')
table = soup.find_all('table')[0]
df = pd.read_html(str(table), flavor='bs4', header=[0])[0]
...
def get_race_urls(year):
    ...
    for url in soup.find_all('a'):
        if (str(year) in str(url.get('href'))) and ('race-result.html' in str(url.get('href'))) and (
                url.get('href') not in race_urls):
...
def seasons_results(race_urls):
    ...
    race_count = None
    with alive_bar(len(race_urls)) as bar:
    ...
    # print(season_results_df) season_results_df['Car'] = season_results_df['Car'].apply(lambda s : s[:3]).map(
    # str.upper) #retain last 3 digits and caps
    if race_count is None:
        race_count = len(season_results_df)
...
color = {'HAM': '#00d2be', 'VER': '#0600ef', 'NOR': '#ff8700', 'LEC': '#dc0000', 'BOT': '#00d2be', 'RIC': '#ff8700',
         'SAI': '#dc0000', 'PER': '#0600ef', 'GAS': '#2b4562', 'STR': '#006F62', 'TSU': '#2b4562', 'OCO': '#0090ff',
         'ALO': '#0090ff', 'MAZ': '#ffffff', 'RAI': '#900000', 'MSC': '#ffffff', 'LAT': '#005AFF', 'RUS': '#005AFF',
         'VET': '#006F62', 'KUB': '#900000', 'GIO': '#900000'}
style = {'HAM': '-', 'VER': '-', 'NOR': '-', 'LEC': '-', 'BOT': '--', 'RIC': '--', 'SAI': '--', 'PER': '--', 'GAS': '-',
         'STR': '--', 'TSU': '--', 'OCO': '--', 'ALO': '-', 'MAZ': '--', 'RAI': '-', 'MSC': '-', 'LAT': '-', 'RUS': '-',
         'VET': '-', 'KUB': ':', 'GIO': '-'}

# preparing data and plotting cumulative point in season
...
ax = cum_results.plot(color=[color.get(x, '#33333') for x in cum_results.columns],
                      style=[style.get(x, '-') for x in cum_results.columns], figsize=(11.5, 7), use_index=True,
                      xlim=(0, race_count - 1))
ax.legend(loc='center left',
          bbox_to_anchor=(1.0, 0.5))
ax.set(xlabel="Races", ylabel="Total points scored", title="Point throughout the season")

# preparing data for moving average filter and
# plotted from index == 2, as 3 frame windows is used and first 2 index are useless
rol_results = results
...
ax2 = rol_results.plot(color=[color.get(x, '#33333') for x in rol_results.columns],
                       style=[style.get(x, '-') for x in rol_results.columns], figsize=(11.5, 7), use_index=False,
                       xlim=(2, race_count - 1))
ax2.legend(loc='center left', bbox_to_anchor=(1.0, 0.5))
ax2.set(xlabel="Races", ylabel="Sum of 3 races",
        title="Moving average throughout the season")  # plt.grid(b=True, which='both', color='0.15s', linestyle='--')
plt.show()
